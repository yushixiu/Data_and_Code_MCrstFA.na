...
#mcrstfa-ECME
MCrstFA.na.ECME=function(Y.na,initial,true.clus,eqnu=F,tol=1e-6,max.iter=100,per=10)
{
  method="ECME"
  begin=proc.time()[1]
  n=nrow(Y.na)
  ...
  CMQ.nu.fn=function(Y,z,mu,S,la,nu)
  {
    logsum=c()
    for(j in 1:num.class.na)
    {
      ...
      mu.o=as.vector(O%*%mu)
      alpha.o=O%*%la
      logsum[j]=sum(z[ind]*drMST(Y.o,mu.o,OSO,alpha.o,nu,log=T))
    }
    -sum(logsum)
  }
  
  CML.nu.fn=function(Y,w,mu,S,la,nu)
  {
    n=nrow(Y)
    wden=matrix(NA,n,g)
    for(i in 1:g)
  ...
  iter=0
  cat(paste(rep("-",20),sep="",collapse=""),method,":\t MCrSTFA.na: (g=",g,"; p=",p,"; q=",q,")","\n")
  ...
  repeat
  {
    ...
    if(eqnu==F)
    {
      for(i in 1:g)
      {
        nu[i]=try(optim(par=nu[i],fn=CMQ.nu.fn,method="L-BFGS-B",lower=2,upper=200,Y=Y,z=z[,i],mu=mu[,i],S=Sig.g[,,i],la=alpha[,i])$par,silent=F)
      }
    }
    else{
      nu=rep(optim(par=nu[1],fn=CML.nu.fn,method="L-BFGS-B",lower=2,upper=200,Y=Y,w=w,mu=mu,S=Sig.g,la=alpha)$par,g)
    }
  ...
  if(eqnu==T) m=(g-1)+q*g+(p*q-q^2)+p+1/2*q*(q+1)*g+q*g+1
  else {m=(g-1)+q*g+(p*q-q^2)+p+1/2*q*(q+1)*g+q*g+g}
