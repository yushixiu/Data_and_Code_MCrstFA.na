...
#mctfa-ECME
MCtFA.na.ECME=function(Y.na,initial,true.clus,eqnu=F,tol=1e-6,max.iter=100,per=10)
{
  method="ECME"
  begin=proc.time()[1]
  n=nrow(Y.na)
  ...
  iter=0
  cat(paste(rep("-",20),sep="",collapse=""),method,":\t MCtFA.na: (g=",g,"; p=",p,"; q=",q,")","\n")
  ...
  if(eqnu==T) m=(g-1)+q*g+(p*q-q^2)+p+1/2*q*(q+1)*g+1
  else {m=(g-1)+q*g+(p*q-q^2)+p+1/2*q*(q+1)*g+g}
  ...
  #mse.mis=mean((Y[is.na(Y.na)==T]-Y.mis)^2)
