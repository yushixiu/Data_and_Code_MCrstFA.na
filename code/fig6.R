...
points3D(x=G1[,1],y=G1[,2],z=G1[,5],bty = 'b2',colkey = F,
         ...
         zlim=c(min(factor[,5]),max(factor[,5])))
points3D(x=G2[,1],y=G2[,2],z=G2[,5],pch=sym[2],add=T,col=colors[1],cex=0.7)
points3D(x=G3[,1],y=G3[,2],z=G3[,5],pch=sym[3], add=T,col=colors[3],cex=0.7)
points3D(x=G4[,1],y=G4[,2],z=G4[,5],pch=sym[4], add=T,col=colors[4],cex=0.7)
points3D(x=G5[,1],y=G5[,2],z=G5[,5],pch=sym[5], add=T,col=colors[5],cex=0.7)
...
b=ggplot(fa2, aes(x=fa2[,1], y=fa2[,5], group=as.factor(p.clus))) + 
  geom_point(aes(color=as.factor(p.clus), shape=as.factor(p.clus)), size=1) +
  scale_colour_manual(values = colors2) +
  scale_shape_manual(values=sym) +
  theme_bw()+
  theme(legend.position='none')+
  labs(y='factor 5',x='factor 1')
b=b+labs(shape = 'Cluster', colour = 'Cluster')+ theme(aspect.ratio=1)
c=ggplot(fa2, aes(x=fa2[,2], y=fa2[,5], group=as.factor(p.clus))) + 
  geom_point(aes(color=as.factor(p.clus), shape=as.factor(p.clus)), size=1) +
  scale_colour_manual(values = colors2) +
  scale_shape_manual(values=sym) +
  theme_bw()+
  theme(legend.position='none')+
  labs(y='factor 5',x='factor 2')
